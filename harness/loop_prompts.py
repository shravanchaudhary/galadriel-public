...
def reflection_prompt(today: str) -> str:
    return (
        ...
        "     - BAD-TRIGGER (high retrieval, rarely used, and the rare uses "
        ...
        "palace_kg_invalidate the stale triple, then palace_kg_add the "
        "replacement (preserves history, mirrors how a fact supersession is "
        "meant to work). For a drawer/procedure/preference: propose_memory "
        ...
        "handles that.\n"
        "     - STALE (retrieved before, unused 90+ days): reassess "
        ...
        "Genuinely obsolete: palace_kg_invalidate for a KG fact, or "
        ...
        "and lesson in room=episodes via palace_add_drawer.\n\n"
        ...
        "lexical-only) → Stage-2 verifies steering value on `matched_chunk` "
        "(embedding max(pos)−max(neg) + junk filter for bare tools/HTML) → "
        "only verified fires inject as a `[Recall detected]` note. Defaults: "
        "positive_threshold=0.6; negatives NEVER gate Stage-1. Always "
...
def task_consolidation_prompt() -> str:
    ...
    return (
        ...
        "positive_threshold + lexical cues propose, Stage-2 embedding "
        "max(pos)-max(neg) + junk filter verifies): learn_recall, "
