...
def reflection_prompt(today: str) -> str:
    return (
        ...
        "     Pipeline: Stage-1 (per-recall positive_threshold / negative_threshold + "
        "lexical) proposes → Stage-2 (local SLM) verifies intent on `matched_chunk` "
        "using that recall's positive/negative examples as YES/NO few-shots → only "
        "verified fires inject. Defaults: positive_threshold=0.6, negative_threshold=0.6 "
        "(absolute: accept if pos>=pos_thr; veto if neg>=neg_thr — not pos vs neg; "
        "lexical hard-hits skip neg thr). "
        ...
        "firing learn_recall or jobs): get_recall → append the short "
        "`matched_chunk` to negative_examples (keep ~3–8 strong near-misses; "
        "drop weakest if needed) and/or trim weak positives/lexical and/or raise "
        "positive_threshold / lower negative_threshold → "
        "`learn_recall(recall_id, …)` with COMPLETE arrays (full replace).\n"
        ...
        "     Cue rules: positives = 3–5 realistic phrasings (not instruction "
        "paraphrases); lexical = high-precision anchors; negatives = near-misses "
        "(these feed Stage-1 veto AND Stage-2 NO few-shots). Thresholds are "
        "visible on get_recall — tune them when cue edits alone cannot fix FP/FN. "
...
def recall_update_prompt() -> str:
    ...
    return (
        ...
        "not written by you as normal replies. (History may still show old assistant prose\n"
        "for those fires; trust the appendix, not role labels.)\n"
        "Also call get_recent_recalls. Two-stage pipeline: Stage-1 (per-recall\n"
        "positive_threshold / negative_threshold + lexical) proposes; Stage-2 (SLM)\n"
        "verifies on matched_chunk using that recall's positive/negative examples as\n"
        "YES/NO few-shots. Cue and threshold patches retune BOTH stages.\n"
        "Defaults 0.6/0.6; accept if pos>=pos_thr; veto if neg>=neg_thr (absolute).\n"
        ...
        "needed, then learn_recall with instruction PLUS non-empty positive_examples,\n"
        "negative_examples, and lexical_cues — you invent the cues; the tool does not.\n"
        "Cue quality (critical — bad cues waste every future turn):\n"
        "- positive_examples: 3–5 short realistic phrasings that should fire (NOT paraphrases of the instruction)\n"
        "- lexical_cues: high-precision exact anchors that should hard-hit\n"
        "- negative_examples: near-misses (Stage-1 veto AND Stage-2 NO few-shots)\n"
        "- positive_threshold / negative_threshold: 0–1 Stage-1 dials (default 0.6); visible on get_recall\n"
        ...
        "- True positive (verified, correct intent): leave alone, or get_recall → append\n"
        "  novel good trigger to positive/lexical → learn_recall(recall_id, arrays…).\n"
        "- False positive (verified/injected but wrong intent): get_recall → append the\n"
        "  short matched_chunk to negative_examples (keep ~3–8 strong near-misses; drop\n"
        "  weakest if needed) and/or trim weak positives/lexical and/or raise\n"
        "  positive_threshold / lower negative_threshold → learn_recall full replace.\n"
        ...
        "Tools available: get_recall, get_recent_recalls, learn_recall, purge_recall only.\n"
    )
