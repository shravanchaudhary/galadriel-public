...
def reflection_prompt(today: str) -> str:
    return (
        ...
        "with the corrected version and a note naming what it supersedes — "
        "there is no drawer-delete tool, so a corrected re-file plus that "
        "note is the closest equivalent (propose_memory's own dedupe keeps "
        "this from duplicating if the correction turns out to already be on "
        "file).\n"
