...
def reflection_prompt(today: str) -> str:
    return (
        ...
        "the stable prompt.\n"
        "  4. RECALL TUNING (two-stage pipeline — cue patches retune Stage-2):\n"
        "     Pipeline: Stage-1 (embed floor / lexical) proposes → Stage-2 (local SLM) "
        "verifies intent on `matched_chunk` using that recall's positive/negative "
        "examples as YES/NO few-shots → only verified fires inject. "
        ...
        "drop weakest if needed) and/or trim weak positives/lexical → "
        ...
        "way. A miss with no proposal at all is also an FN → strengthen or create.\n"
        ...
        "(these feed Stage-1 veto AND Stage-2 NO few-shots). Prefer patching an "
        "existing recall; `purge_recall` only for bad user recalls. System recall "
        "instructions are immutable — cue arrays only. Do not essay facts into "
        "instructions.\n\n"
...
def recall_update_prompt() -> str:
    ...
    return (
        ...
        "Also call get_recent_recalls. Two-stage pipeline: Stage-1 (embed/lexical)\n"
        "proposes; Stage-2 (SLM) verifies on matched_chunk using that recall's\n"
        "positive/negative examples as YES/NO few-shots. Cue patches retune BOTH stages.\n"
        ...
        "Package: durable content → palace_add_drawer / palace_kg_add; when-to-recollect →\n"
        "learn_recall pointing at that store. If yes and it should fire again later via a\n"
        "short lookup: get_recall as needed, then learn_recall with instruction PLUS\n"
        "non-empty positive_examples, negative_examples, and lexical_cues — you invent the\n"
        "cues; the tool does not.\n"
        ...
        "  weakest if needed) and/or trim weak positives/lexical → learn_recall full\n"
        "  replace. User recalls may also tighten instruction.\n"
        ...
        "- Miss with no proposal: get_recall → strengthen positives/lexical, or\n"
        "  learn_recall create if missing.\n\n"
