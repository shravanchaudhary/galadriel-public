...
def reflection_prompt(today: str) -> str:
    return (
        ...
        "the stable prompt.\n"
        "  4. RECALL TUNING: Run `get_recent_recalls` to see recent recall fires, and "
        "`get_recall` for definitions. For each fire, gauge if it was helpful. "
        "True positive / false negative: `get_recall` then `learn_recall(recall_id, …)` "
        "with the COMPLETE intended cue arrays (full replace — never partial). "
        "False positive: add a near-miss to negative_examples (and/or trim positives/"
        "lexical) via the same `learn_recall` patch. You may also `learn_recall` to "
        "create a new recall when a cross-cutting pattern deserves one, or "
        "`purge_recall` for a bad user recall. System recall instructions are "
        "immutable — cue arrays only. Negatives are live vetoes at match time.\n\n"
...
def recall_update_prompt() -> str:
    ...
    return (
        ...
        "If yes and it should fire again later via a short lookup: get_recall as needed,\n"
        "then learn_recall with instruction PLUS non-empty positive_examples,\n"
        ...
        "PART B — Audit recalls that already fired (and misses)\n"
        "For each recall fire listed in the appendix (and optionally get_recent_recalls):\n"
        "- True positive: leave alone, or get_recall → append novel good trigger to "
        "positive/lexical → learn_recall(recall_id, arrays…).\n"
        "- False positive: get_recall → add near-miss to negative_examples (and/or trim "
        "positives/lexical) → learn_recall(recall_id, arrays…). User recalls may also "
        "tighten instruction.\n"
        "- False negative (should have fired, did not): get_recall → strengthen "
        "positives/lexical, or learn_recall create if missing.\n\n"
