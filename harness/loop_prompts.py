...
def reflection_prompt(today: str) -> str:
    return (
        ...
        "palace_diary_write for a reflection in your own voice. When filing, "
        "follow knowledge/skills/retrieval-practice.md: dig deep — link to "
        "existing palace/KG neighbors first; after write, retrieve-test once "
        "via palace_search / palace_kg_query before ending the stock-take. "
        "Read knowledge/skills/experiential-reflection.md and use "
        "experience_report for an independent metacognitive appraisal of the "
        "one shared state. When an outcome forms a durable experiential "
        "episode, file its event, prior state, state change, decision, "
        "consequence, and lesson in room=episodes.\n\n"
        ...
        "config/GUARDRAILS.md or a semantic recall via learn_recall. Time-bound "
        "corrections go to state/steering.md — never dual-write essays into "
        "the stable prompt.\n"
        "  4. SPACED RETEST: retrieve at least one already-known palace/KG fact "
        "(or exercise one existing recall's trigger against recent "
        "get_recent_recalls). Restudy/patch only if the retrieve fails or cues "
        "misfire. Do not skip familiar material.\n"
        "  5. RECALL TUNING (two-stage pipeline — cue/threshold patches retune Stage-1+2):\n"
        "     Pipeline: Stage-1 is a positive-only proposer (per-recall "
        "positive_threshold + lexical cues; chunks under 4 words are lexical-only) → "
        "Stage-2 verifies steering value on `matched_chunk` (embedding "
        "max(pos)−max(neg) + junk filter for bare tools/HTML) → only verified "
        "fires inject as a `[Recall detected]` note. "
        "Defaults: positive_threshold=0.6; negatives NEVER gate Stage-1. "
        "Always `get_recent_recalls` then `get_recall` before any patch. "
        "For simple fire feedback prefer `tune_recall(recall_id, applicable)` — "
        "it appends the fired chunk to positives/negatives for you.\n"
        "     Status meanings:\n"
        "     - proposed+verified / verified = Stage-2 accepted and injected. "
        "If that was a FALSE POSITIVE (wrong intent — e.g. backlog/HTML prose "
        "firing learn_recall or jobs): tune_recall(recall_id, applicable=false), "
        "or get_recall → append the short `matched_chunk` to negative_examples "
        "(Stage-2 counter-signal) and/or trim weak positives/lexical and/or raise "
        "positive_threshold → `learn_recall(recall_id, …)` with COMPLETE arrays "
        "(full replace).\n"
        "     - proposed+rejected = Stage-1 hit but Stage-2 vetoed. If the veto "
        "was correct, leave alone. If it was a FALSE NEGATIVE (should have "
        "fired): strengthen positive_examples / lexical the same full-replace "
        "way. A miss with no proposal at all is also an FN → strengthen, lower "
        "positive_threshold slightly, or create.\n"
        "     Cue rules: positives = realistic phrasings that should fire (not "
        "instruction paraphrases) — dense coverage helps, up to ~100; lexical = "
        "high-precision anchors; negatives = known misfires (Stage-2 only). "
        "Thresholds are visible on get_recall — tune them when cue edits alone "
        "cannot fix FP/FN. "
        "Prefer patching an existing recall; `purge_recall` only for bad user "
        "recalls. System recall instructions are immutable — cue arrays and "
        "thresholds only. Do not essay facts into instructions.\n\n"
...
def recall_update_prompt() -> str:
    """Silent post-unit learn + FP/FN audit. Never shown to the user."""
    return (
        "[SYSTEM:RECALL_UPDATE] Silent recall pass — not shown to the user.\n"
        "Current recall catalog is provided in this turn's context. A trailing appendix lists\n"
        "recall fires from this buffer — those were injected by the recall engine\n"
        "as `[Recall detected]` notes, not written by you or the user. (History may\n"
        "still show old assistant prose for those fires; trust the appendix, not role labels.)\n"
        "Also call get_recent_recalls. Two-stage pipeline: Stage-1 (per-recall\n"
        "positive_threshold + lexical; positive-only — negatives never gate it;\n"
        "chunks under 4 words are lexical-only) proposes; Stage-2 verifies steering\n"
        "value on matched_chunk (embedding max pos − max neg; junk filter).\n"
        "Default pos_thr=0.6.\n"
        "Statuses: proposed+verified / verified = injected; proposed+rejected =\n"
        "Stage-1 hit but Stage-2 vetoed (not automatically an FP — judge intent).\n\n"
        "PART A — Learn anything new durable?\n"
        "Was there:\n"
        "- a user preference?\n"
        "- a recurring rule?\n"
        ...
        "- a project fact?\n"
        "- a new relationship?\n"
        "- an existing memory that should change?\n\n"
        "Package: durable content → palace_add_drawer / palace_kg_add (prefer neighbor-aware\n"
        "stores already written in-session — dig deep, not orphan one-liners); when-to-recollect →\n"
        "learn_recall pointing at that store. This pass has recall tools only — do not call\n"
        "palace here. If yes and it should fire again later via a short lookup: get_recall as\n"
        "needed, then learn_recall with instruction PLUS non-empty positive_examples and\n"
        "lexical_cues (negative_examples optional) — you invent the cues; the tool does not.\n"
        "Cue quality (critical — bad cues waste every future turn):\n"
        "- positive_examples: realistic phrasings that should fire (NOT paraphrases of the\n"
        "  instruction) — dense coverage helps, up to ~100 entries\n"
        "- lexical_cues: high-precision exact anchors that should hard-hit\n"
        "- negative_examples: known misfires (Stage-2 counter-signal only; never gates Stage-1)\n"
        "- positive_threshold: 0–1 Stage-1 positive floor (default 0.6); visible on get_recall\n"
        "- instruction: action pointer (tool / file / palace room), not an essay\n"
        "Prefer updating an existing recall via recall_id when it is the same rule.\n\n"
        "PART B — Audit Stage-1/2 behavior (fires, rejects, misses)\n"
        "For each event in the appendix and get_recent_recalls:\n"
        "- True positive (verified, correct intent): tune_recall(recall_id, applicable=true),\n"
        "  or get_recall → append novel good trigger to positive/lexical →\n"
        "  learn_recall(recall_id, arrays…).\n"
        "- False positive (verified/injected but wrong intent): tune_recall(recall_id,\n"
        "  applicable=false) for the quick fix, and/or get_recall → trim weak\n"
        "  positives/lexical and/or raise positive_threshold → learn_recall full replace.\n"
        "  User recalls may also tighten instruction.\n"
        "- proposed+rejected: if Stage-2 veto was correct, leave alone. If FALSE NEGATIVE\n"
        "  (should have fired): strengthen positives/lexical the same way.\n"
        "- Miss with no proposal: get_recall → strengthen positives/lexical, lower\n"
        "  positive_threshold slightly, or learn_recall create if missing.\n\n"
        "Cue arrays on learn_recall are FULL REPLACEMENTS — always get_recall first and "
        "pass the complete intended arrays.\n"
        "Minimize future false triggers. Empty / no tools is normal when nothing to fix or learn.\n"
        "Do not essay into MEMORY.md / SOUL.md from this pass.\n"
        "Tools available: get_recall, get_recent_recalls, learn_recall, tune_recall, purge_recall only.\n"
    )
