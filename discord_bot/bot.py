...
def create_bot(agent: GaladrielAgent, scheduler=None, completion_watcher=None, worker=None) -> commands.Bot:
    ...
    async def compact_cmd(ctx: commands.Context):
        """Compress the whole conversation into a structured snapshot (archives first)."""
        if ctx.author.id != AUTHORIZED_USER_ID:
            return
        ...
        async with ctx.channel.typing():
            try:
                result = await agent.compact_channel(channel_id)
                if not result.get("compacted"):
                    await ctx.reply(f"📚 {msg_count} messages — nothing to compact.")
                    return
                ...
                await ctx.reply(
                    f"🗜️ **Compacted to snapshot**\n"
                    f"Messages: {result['messages_before']} → 0\n"
                    f"Tokens: ~{result['tokens_before']} → ~{result['tokens_after']} (~{ratio_pct}% reduction)\n"
                    f"Full conversation archived to palace. A compact snapshot is kept in context."
                )
                log.info(
                    f"Compaction complete: {result['messages_before']} msgs, "
    ...
    async def slash_compact(interaction: discord.Interaction):
        ...
        try:
            result = await agent.compact_channel(channel_id)
            if not result.get("compacted"):
                await interaction.followup.send(f"📚 {msg_count} messages — nothing to compact.")
                return
